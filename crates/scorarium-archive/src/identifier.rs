...
/// One identifier as raw text from the web form
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct IdentifierRawInput {
...
///
/// A publication's identifiers are checked together rather than one at a time because no two of
/// them may normalize to the same thing.
pub(crate) fn parse_identifiers(
    raw: &[IdentifierRawInput],
) -> Result<Vec<(Kind, Normalized)>, Vec<Option<ValidationError>>> {
    ...
    let errors: Vec<Option<ValidationError>> = raw
        ...
        .map(|raw| {
            ...
            let Ok(kind) = raw.kind.trim().parse::<Kind>() else {
                return Some(ValidationError::UnknownIdentifierKind);
            };
            let normalized = match normalize(kind, value) {
                Ok(normalized) => normalized,
                Err(err) => return Some(ValidationError::InvalidIdentifier(err)),
            };
            // The schema is unique on (publication, kind, value), so a second identifier
            // normalizing to the same value would fail the write; refuse it here instead.
